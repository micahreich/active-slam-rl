...
from asrl.realnvp2d.nf.flows import ConditionalMaskedAffineFlow
...
def init_realnvp2d(n_channels, seq_len, n_layers, context_dim=None):
    ...
    for i in range(n_splits):
        for j in range(n_layers):
            s = ResidualTemporalBlock1D(inp_channels=n_channels, out_channels=n_channels,
                                        kernel_size=5, init_zeros=True, context_dim=context_dim)
            t = ResidualTemporalBlock1D(inp_channels=n_channels, out_channels=n_channels,
                                        kernel_size=5, init_zeros=True, context_dim=context_dim)
            ...
            flows_reversed += [ConditionalMaskedAffineFlow(b, t, s)]
