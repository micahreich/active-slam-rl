...
def init_Flow(sigma_max, sigma_min, action_sizes, state_sizes):
    ...
    layer_norm_scale = True
    hidden_layers = 2
    flow_layers = 2
    hidden_sizes = 64
    scale_hidden_sizes = 64
    ...
    log_scale = None #MLP(prior_list, init=init_parameter)
    ...
    for i in range(flow_layers):
        ...
        n_params_per_layer = sum(p.numel() for p in s1.parameters() if p.requires_grad) + \
                             sum(p.numel() for p in t1.parameters() if p.requires_grad)
                             
        print(f"!!! Constructed flow layer {i} with {2 * n_params_per_layer} params")
    ...
    n_params_per_scale = sum(p.numel() for p in learnable_scale_1.parameters() if p.requires_grad)
    print(f"!!! Constructed scale layer with {2 * n_params_per_scale} params")
