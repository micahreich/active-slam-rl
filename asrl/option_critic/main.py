...
from asrl.option_critic.agents.option_critic import OptionCriticFeatures, soft_update
from asrl.option_critic.agents.option_critic import critic_loss as critic_loss_fn
from asrl.option_critic.agents.option_critic import actor_loss as actor_loss_fn
...
parser.add_argument('--env', default='FourRooms-v0', help='ROM to run')
...
parser.add_argument('--epsilon-decay', type=float, default=1e5, help=('Number of steps to minimum epsilon.'))
parser.add_argument('--entropy-start', type=float, default=100, help=('Starting value for entropy.'))
parser.add_argument('--entropy-min', type=float, default=0.1, help='Minimum entropy.')
parser.add_argument('--entropy-decay', type=float, default=1e5, help=('Number of steps to minimum entropy.'))
...
parser.add_argument('--update-frequency', type=int, default=5, help=('Number of actions before each SGD update.'))
...
parser.add_argument('--temp', type=float, default=2.0, help='Action distribution softmax tempurature param.')
...
def run(args):
    ...
    option_critic = option_critic(
        ...
        ent_start=args.entropy_start,
        ent_min=args.entropy_min,
        ent_decay=args.entropy_decay,
    ...
    while steps < args.max_steps_total:
        ...
        while not done and ep_steps < args.max_steps_ep:
            ...
            ent_coeff = option_critic.entropy_coeff
            ...
            if len(buffer) > args.batch_size:
                actor_loss = actor_loss_fn(obs, current_option, logp, entropy, ent_coeff, \
                ...
                optim.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(option_critic.parameters(), max_norm=1.0)
                
                optim.step()

                # soft_update(option_critic_prime, option_critic, tau=0.005)
