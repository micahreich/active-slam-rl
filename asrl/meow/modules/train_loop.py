...
def train_loop(agent, args, buffer, train_envs, test_envs, logger):
    ...
    print(f"Training MEOW with {total_params} params")

    for t in tqdm(range(args.steps + 1),
                  desc="Training",
                  position=0,
                  leave=True):
        ...
        if t >= args.warmup_steps:
            ...
            if t % args.eval_every == 0:
                agent.policy.eval()
                test_return = evaluate(test_envs,
                                       agent,
                                       deterministic=args.deterministic)
                tqdm.write(f"Test return = {test_return:.3f} at step {t}")

                if test_return > best_test_return:
                    best_test_return = test_return
                    torch.save(agent.policy,
                               os.path.join(args.save_path, 'best.pt'))

                    tqdm.write(
                        f"\tSaving agent to {args.save_path} with best return {best_test_return:.3f} at step {t}"
                    )
                log(
                    {
                        **result,
            ...
            if t % args.plot_every == 0:
                ...
                torch.save(agent.policy, os.path.join(args.save_path,
                                                      'best.pt'))
    ...
    print(f"Saved last agent to {args.save_path}")
