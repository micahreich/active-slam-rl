...
def convert_to_shape_tuple(shape):
...
def outputdir_make_and_add(outputdir, title=None):
    ...
    folder_num = len(next(
        os.walk(outputdir))[1])  #counts how many folders already there
    if folder_num == 0:
        folder_num = 1
        ...
        folder_num = max(
            [
                int(i.split('-')[0])
                for i in next(os.walk(outputdir))[1] if i[0] != '.'
            ],
            default=0
        ) + 1  # this looks for max folder num and adds one... this works even if title is used (because we index at 1) (dot check to ignore .ipynb)
    ...
    if title == None:
        ...
        outputdir += '/' + str(
            folder_num) + f'-{title}'  #adds one and appends title
    os.makedirs(outputdir, exist_ok=True)
    return outputdir
...
def plot_traj_multigoal(critic,
                        file_name='_.png',
                        runs=8,
                        deterministic=False,
                        rewards_thres=-10000000,
                        render_plot=True):
    with torch.no_grad():
        paths = []
        rewards = []
        ...
        if render_plot:
            env.unwrapped.render_rollouts(paths, file_name + '.png')
...
def plot_value(critic, file_name='_.png'):
    critic.eval()
    grid_size = 100
    xx, yy = torch.meshgrid(torch.linspace(-8, 8, grid_size),
                            torch.linspace(-8, 8, grid_size),
                            indexing="ij")
    zz = torch.cat([xx.unsqueeze(2), yy.unsqueeze(2)], 2).view(-1,
                                                               2).to('cuda')

    neg_energy = critic.get_v(obs=torch.cat((zz, zz), dim=0))
    neg_energy = neg_energy[:neg_energy.shape[0] //
                            2].view(*xx.shape).detach().cpu().numpy()
