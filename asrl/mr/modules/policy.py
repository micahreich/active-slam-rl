...
def init_flow(sigma_max, sigma_min, action_sizes, state_sizes) -> Tuple[List[Flow], ConditionalDiagLinearGaussian]:
    init_parameter = "zero"
    ...
    context_encoder = ContextEncoderDiagGaussian(action_sizes, log_scale, loc)
    q0 = ConditionalDiagLinearGaussian(action_sizes, context_encoder, sigma_min, sigma_max)
...
class FlowPolicyOLD(nn.Module):
    ...
    def sample(self, num_samples, obs, deterministic=False):
        ...
        if deterministic: # (Warning: This is only implemented for MEow with the additive coupling layers and the Gaussian prior)
            ...
            act, log_q = self.prior.forward(num_samples=num_samples, context=obs)
