...
class MEOW():
    ...
    def __init__(
        ...
        self.buffer = ReplayBuffer(int(args.buffer_size), args.state_size,
                                   args.action_size)
        self.state_size = args.state_size
        self.action_size = args.action_size

        policy = FlowPolicy(args)
    ...
    def train(self, train_envs, test_envs):
        self.unit_test()
        train_loop(self, self.args, self.buffer, train_envs, test_envs,
                   self.logger)

    def unit_test(self):
        self.policy.eval()
        num_samples = 10
        state = torch.randn((num_samples, self.state_size), device=self.device)
        action = torch.clamp(torch.randn((num_samples, self.action_size),
                                         device=self.device),
                             min=-1,
                             max=1)
    ...
    def act(self, obs, deterministic=False):
        ...
        act, log_prob = self.policy.sample(num_samples, obs, deterministic)
    ...
    def update_q(self, batch, mseloss=nn.MSELoss(reduction='none')):
        ...
        with torch.no_grad():
            ...
            exact_v_old = torch.min(v_old[:v_old.shape[0] // 2],
                                    v_old[v_old.shape[0] // 2:])
            target_q = r + (1 - d) * self.args.gamma * exact_v_old

        self.policy.train()
        current_q1, _ = self.policy.get_qv(torch.cat((s, s), dim=0),
                                           torch.cat((a, a), dim=0))
        ...
        for p in list(
            ...
            weights = torch.cat(
                (weights, p.grad.flatten()),
                0) if weights is not None else p.grad.flatten()
    ...
    def update(self):
        ...
        (s, a, r, s_n, d) = batch['states'], batch['actions'], batch[
            'rewards'], batch['next_states'], batch['dones']
        # Update policy
    ...
    def soft_update(self, tgt: nn.Module, src: nn.Module):
        for tgt_param, src_param in zip(tgt.parameters(), src.parameters()):
            tgt_param.data.copy_(self.args.tau * src_param.data +
                                 (1 - self.args.tau) * tgt_param.data)
