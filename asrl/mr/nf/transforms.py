...
class Preprocessing(Flow):
    def __init__(self):
        ...
        eps = 1e-6
        self.transform = ComposeTransform(parts=[TanhTransform(), Clip(-1+eps, 1-eps)])

    def forward(self, z, context=None):
        # clip_lo = self.tanh_transform.codomain.lower_bound + 1e-5
        # clip_hi = self.tanh_transform.codomain.upper_bound - 1e-5
        
        _z = self.transform(z)
        log_det = self.transform.log_abs_det_jacobian(z, _z).sum(-1, keepdim=False)
    ...
    def inverse(self, z, context=None):
        ...
        log_det = self.transform.inv.log_abs_det_jacobian(z, _z).sum(-1, keepdim=False)
